...
<Note>
  The Compose files use Docker Hub for all base images
  (`postgres:16-alpine`, `node:20-alpine`) for reliability and global CDN
  distribution. Set `BYTEBOT_POSTGRES_IMAGE` if you prefer a different registry.
</Note>
...
<AccordionGroup>
  ...
  <Accordion title="Using LiteLLM Proxy">
    ```bash
    # To use multiple LLM providers, use the proxy setup (build locally by default):
    docker-compose -f docker/docker-compose.proxy.yml up -d --build

    # Optional: use registry images after authenticating
