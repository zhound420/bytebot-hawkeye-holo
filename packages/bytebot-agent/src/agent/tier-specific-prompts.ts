...
export function getTierSpecificCVInstructions(
   ...
   **YOU MUST FOLLOW THIS WORKFLOW FOR ALL UI CLICKS:**
   ...
   ✅ **REQUIRED WORKFLOW (Your model has STRONG CV capabilities):**
   ...
   ✅ **YOUR MODEL STRENGTH:** Your model has strong vision capabilities. CV detection should work reliably for most UI elements. Trust the CV tools.

   **This is MANDATORY, not optional.** computer_detect_elements + computer_click_element has 89% accuracy vs 60% for manual grid clicking. Your model excels at this - always use CV tools first.`;
  }
   ...
   **STRONGLY RECOMMENDED WORKFLOW FOR ALL UI CLICKS:**
   ...
   ✅ **RECOMMENDED WORKFLOW (Your model has GOOD CV capabilities):**
   1. Take screenshot with computer_screenshot
   2. Detect elements with computer_detect_elements({ description: "target element" })
   3. Click using computer_click_element({ element_id: "..." })
   ...
   **BALANCED APPROACH:** Your model has good vision capabilities. CV detection works well for most elements, but keyboard shortcuts are a reliable fallback for tricky cases.
  ...
  // Tier 3: Weak CV capability - Minimal enforcement, keyboard-first
   ...
   **RECOMMENDED APPROACH FOR UI INTERACTION:**
   ...
   ⚠️ **YOUR MODEL NOTE:** Your model may struggle with CV detection in some cases. Keyboard shortcuts are more reliable for your tier. Use CV detection as an assist, not primary method.
...
export function getTierSpecificUIMethodsSection(
  ...
  // Tier 1: Strong CV - CV-first emphasis
  if (tier === 'tier1') {
    return `### UI Element Interaction - CV-First Approach

**IMPORTANT: Always use Method 1 (CV-Assisted) for clicking UI elements. Your model has STRONG CV capabilities.**
...
- ✅ YOUR MODEL EXCELS: Strong vision capabilities make CV detection highly reliable
...
Use ONLY when Method 1 has failed ${maxCvAttempts}+ times for the same element.`;
  ...
  // Tier 2: Medium CV - Balanced approach
  if (tier === 'tier2') {
    return `### UI Element Interaction - Balanced Approach

**RECOMMENDED: Try Method 1 (CV-Assisted) first, fallback to Method 2 (Keyboard) if needed. Your model has GOOD CV capabilities.**
...
**Why CV-First:**
- ✅ 89% success rate vs 60% with manual grid clicking
- ✅ YOUR MODEL: Good vision capabilities, works well for most elements
- ⚠️ May struggle with complex/ambiguous UI elements
...
Use ONLY when both Method 1 and Method 2 have failed.`;
  ...
  // Tier 3: Weak CV - Keyboard-first emphasis
  return `### UI Element Interaction - Keyboard-First Approach

**YOUR MODEL NOTE: Your model may struggle with CV detection. Keyboard shortcuts are MORE RELIABLE for your tier.**
...
⚠️ **LIMITATION:** Your model tier may struggle with CV detection. If detection fails ${maxCvAttempts} times, don't persist - fallback to Method 3.
